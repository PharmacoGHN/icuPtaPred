...
pta_hover_text <- function(mic, ratio, label) {
  paste0(
    "Minimum inhibitory concentration: ", format_plot_value(mic), " mg/L",
    "<br>", label,
    "<br>Steady-state concentration to MIC ratio: ", format_plot_value(ratio)
  )
}
...
plot.pta <- function(data, ecoff = NA, selected_dose = NA_real_, dose_increment = NA_real_) {
  ...
  pta_plot <- ggplot2$ggplot(data = data) +
    ...
    ggplot2$geom_line(mapping = ggplot2$aes(x = .data$mic, y = .data$css_mic, text = .data$selected_hover), col = "#2db391", lty = 1, lwd = 1, na.rm = TRUE) +
  ...
  if (any(is.finite(data$toxicity_threshold) & !is.na(data$toxicity_threshold))) {
    pta_plot <- pta_plot + ggplot2$geom_line(
      mapping = ggplot2$aes(x = .data$mic, y = .data$toxicity_threshold, text = .data$toxicity_hover),
      col = "#960b0b",
      lty = 1,
      lwd = 0.9,
      na.rm = TRUE
  ...
  if (any(is.finite(data$additional_threshold) & !is.na(data$additional_threshold))) {
    pta_plot <- pta_plot + ggplot2$geom_line(
      mapping = ggplot2$aes(x = .data$mic, y = .data$additional_threshold, text = .data$additional_hover),
      col = "#f2a65a",
      lty = 3,
      lwd = 0.9,
      na.rm = TRUE
  ...
  pta_multiple_doses <- pta_plot +
    ggplot2$geom_line(
      data = data,
      mapping = ggplot2$aes(x = .data$mic, y = .data$css_mic_below1, text = .data$below1_hover),
      col = "#20846b",
      lty = 1,
      lwd = 1,
      na.rm = TRUE
    ...
    ggplot2$geom_line(
      data = data,
      mapping = ggplot2$aes(x = .data$mic, y = .data$css_mic_below2, text = .data$below2_hover),
      col = "#1f8269",
      lty = 1,
      lwd = 1,
      na.rm = TRUE
    ...
    ggplot2$geom_line(
      data = data,
      mapping = ggplot2$aes(x = .data$mic, y = .data$css_mic_above1, text = .data$above1_hover),
      col = "#32c5a0",
      lty = 1,
      lwd = 1,
      na.rm = TRUE
    ...
    ggplot2$geom_line(
      data = data,
      mapping = ggplot2$aes(x = .data$mic, y = .data$css_mic_above2, text = .data$above2_hover),
      col = "#2fe3b6",
      lty = 1,
      lwd = 1,
      na.rm = TRUE
...
plot.cfr <- function(data, dose_increment = 0) {
  ...
  cfr_plot <- ggplot2$ggplot(data, ggplot2$aes(x = .data$dose / 1000)) +
    ggplot2$geom_line(ggplot2$aes(y = .data$cfr, text = .data$cfr_hover), col = "#2db391", lty = 1, lwd = 1)

  if (any(is.finite(data$toxicity_proportion) & !is.na(data$toxicity_proportion))) {
    cfr_plot <- cfr_plot + ggplot2$geom_line(
      ggplot2$aes(y = .data$toxicity_proportion, text = .data$toxicity_hover),
      col = "#960b0b"
    )
  }
